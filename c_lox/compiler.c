...
static uint8_t findStringConstant(const char* start, int length) {
    for (int i = 0; i < currentChunk()->constants.count; i++) {
        if (!IS_STRING(currentChunk()->constants.values[i])) continue;
...
static void dot(bool canAssign) {
    if (match(TOKEN_LEFT_BOX)) {  
        expression();
        consume(TOKEN_RIGHT_BOX, "Expect ']' after property name.");
...
ParseRule rules[] = {
    ...
    [TOKEN_LEFT_BOX]      = {NULL,     NULL,   PREC_NONE},
    [TOKEN_RIGHT_BOX]     = {NULL,     NULL,   PREC_NONE},
